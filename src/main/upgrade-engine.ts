...
export function restoreFromBackup(projectPath: string, backupPath: string): RollbackResult {
  ...
  try {
    ...
    try {
      ...
      log.push('[ERROR] Backup manifest is corrupted (invalid JSON)')
      return { success: false, log, restoredFiles }
    }

    const files = Array.isArray(manifest.files) ? manifest.files : []
    if (files.length === 0) {
      log.push('[ERROR] Backup manifest contains no files')
      return { success: false, log, restoredFiles }
    }
...
export function applyUpgrade(
  ...
  const metaRelPath = '.claude/.hal-o-meta.json'
  if (existsSync(join(projectPath, metaRelPath))) {
    filesToBackup.push(metaRelPath)
  }
  ...
  try {
    ...
    // 3. Update meta — ensure directory exists first
    const metaPath = join(projectPath, '.claude', '.hal-o-meta.json')
 ...
 */
function mergeClaudeMdSections(existing: string, newGenerated: string): string {
  // Strategy 1: Replace <!-- HAL-O additions --> block (legacy append format)
  ...
  // Strategy 2: Replace versioned append block
  ...
  // Strategy 3: Full HAL-O CLAUDE.md (has versioned section markers like header, stack, etc.)
  // Replace each versioned section in-place, preserving any user content between sections
  const versionedHeader = existing.match(/<!-- hal-o:v\d+:header -->/)
  const versionedFooter = existing.match(/<!-- hal-o:v\d+:footer -->/)

  if (versionedHeader && versionedFooter) {
    // The entire file was generated by HAL-O — replace it wholesale with the new version
    // but preserve any user content that appears BEFORE the first marker
    const headerIdx = existing.indexOf(versionedHeader[0])
    const footerMatch = existing.match(/<!-- hal-o:v\d+:footer -->/)
    if (headerIdx !== -1 && footerMatch) {
      const footerIdx = existing.indexOf(footerMatch[0])
      const userPreamble = existing.slice(0, headerIdx).trim()
      const userPostamble = existing.slice(footerIdx + footerMatch[0].length).trim()

      let merged = ''
      if (userPreamble) merged += userPreamble + '\n\n'
      merged += newGenerated
      if (userPostamble) merged += '\n\n' + userPostamble
      return merged
    }
  }
...
function detectUserCustomizations(content: string): boolean {
  ...
  let cleaned = content
  // Remove HAL-O sections — strip all marker comments and known generated blocks
  cleaned = cleaned.replace(/<!-- hal-o:v\d+:[\w-]+ -->/g, '')
  cleaned = cleaned.replace(/<!-- HAL-O additions -->[\s\S]*?<!-- \/HAL-O additions -->/g, '')
  cleaned = cleaned.replace(/<!-- hal-o:v\d+:append-start -->[\s\S]*?<!-- hal-o:v\d+:append-end -->/g, '')
