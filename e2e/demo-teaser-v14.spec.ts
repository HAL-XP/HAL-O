/**
 * Demo Teaser v14 — GDIGRAB real-time capture + camera intro spline.
 *
 * KEY CHANGES from v12:
 *   - Camera approach: [0, 8, 20] → [0, 2.5, 11] over 3s (lower Y for ring 0 centering)
 ...
 *
 * Run:
 *   npx playwright test e2e/demo-teaser-v14.spec.ts --timeout 300000
 */
...
const RAW_V14 = resolve(TEMP, 'demo-v14-raw.mp4')
const FINAL_V14 = resolve(TEMP, 'demo-v14.mp4')
const GREETING_WAV = 'C:/Users/dindo/AppData/Local/Temp/hal-v10.wav'
const TIMING_LOG = resolve(TEMP, 'demo-v14-timing.json')
...
// ===================================================================
// TEST: v14 — 16s teaser, GDIGRAB + camera intro spline + auto-rotate
// ===================================================================
test('Video v14 — 16s teaser, GDIGRAB + camera intro spline + auto-rotate', async () => {
  ;({ app, page } = await launchApp())
  await setupScene()

  // Clean old output files
  for (const f of [RAW_V14, FINAL_V14]) {
  ...
  const rawPath = RAW_V14.replace(/\\/g, '/')
  const ffmpegArgs = [
    '-f', 'gdigrab',
    '-framerate', '30',
    '-offset_x', '0',
  ...
  // ── Start camera approach animation: far establishing → ring 0 position over 3s ──
  log('Camera approach animation starting: [0, 8, 20] → [0, 2.5, 11] over 3s')
  await page.evaluate(() => {
    ;(window as any).__haloPhotoMode.animateCamera([
      { t: 0,    pos: [0, 8, 20] as [number, number, number] },
      { t: 3000, pos: [0, 2.5, 11] as [number, number, number] },
    ])
  })
  ...
  await page.evaluate((url: string) => {
    ...
    a.volume = 1.0
    a.play().catch(e => console.error('[v14] Audio play failed:', e))
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    ffmpegProc.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        console.error('[v14] ffmpeg stderr (last 1000 chars):', ffmpegStderr.slice(-1000))
  ...
  if (existsSync(RAW_V14)) {
    const rawSize = (statSync(RAW_V14).size / 1024 / 1024).toFixed(1)
  ...
  const finalPath = FINAL_V14.replace(/\\/g, '/')
  ...
  if (existsSync(GREETING_WAV)) {
    ...
    try {
      ...
      console.error('[v14] Scale+audio failed:', err.stderr?.toString().slice(-500))
  ...
  // ── Check final file ──
  const outputPath = existsSync(FINAL_V14) ? FINAL_V14 : RAW_V14
  ...
  for (const ts of qaTimestamps) {
    const qaFrame = resolve(TEMP, `demo-v14-qa-${ts}s.jpg`).replace(/\\/g, '/')
    try {
      execSync(
  ...
  writeFileSync(TIMING_LOG, JSON.stringify({
    ...
    camera: {
      approach_start: [0, 8, 20],
      approach_end: [0, 2.5, 11],
      approach_duration_ms: 3000,
      auto_rotate_speed: 0.12,
    },
    video: {
      raw_video: 'demo-v14-raw.mp4',
      final_video: 'demo-v14.mp4',
  ...
  // ── QA summary ──
  console.log('\n[v14] QA CHECKLIST:')
  console.log('  [ ] 0s: Far establishing shot (camera at [0, 8, 20])')
  console.log('  [ ] 1.5s: Camera approaching — mid-flight')
  console.log('  [ ] 2s: Audio starts (butler greeting)')
  console.log('  [ ] 3s: Arrived at ring 0 [0, 2.5, 11] — cards centered vertically')
