/**
 * Demo Teaser v12 — camera approach intro + closer to cards.
 *
 * KEY CHANGES from v11:
 *   - Camera approach: start far [0, 10, 20], glide to [0, 4, 11] over 3s
 *   - Final position CLOSER: Y=4, Z=11 (was Y=5, Z=14 — too far)
 *   - After approach, auto-rotate takes over seamlessly (no teleport)
 *   - Total: 16s (3s approach + 13s auto-rotate)
 *
 * Run:
 *   npx playwright test e2e/demo-teaser-v12.spec.ts --timeout 300000
 */
...
const RAW_V12 = resolve(TEMP, 'demo-v12-raw.mp4')
const FINAL_V12 = resolve(TEMP, 'demo-v12.mp4')
const GREETING_WAV = 'C:/Users/dindo/AppData/Local/Temp/hal-v10.wav'
const TIMING_LOG = resolve(TEMP, 'demo-v12-timing.json')
...
// ===================================================================
// TEST: v12 — 16s teaser, camera approach + closer cards
// ===================================================================
test('Video v12 — 16s teaser, camera approach intro + closer to cards', async () => {
  ;({ app, page } = await launchApp())
  await setupScene()

  // Clean old output files
  for (const f of [RAW_V12, FINAL_V12]) {
  ...
  await sleep(200)
  log('Auto-rotate disabled for approach animation')
  ...
  const rawPath = RAW_V12.replace(/\\/g, '/')
  const ffmpegArgs = [
    '-f', 'gdigrab',
    '-framerate', '30',
    '-offset_x', '0',
  ...
  // ── Start camera approach animation: far → close over 3s ──
  log('Camera approach animation starting: [0,10,20] → [0,4,11] over 3s')
  await page.evaluate(() => {
    ;(window as any).__haloPhotoMode.animateCamera([
      { t: 0,    pos: [0, 10, 20] as [number, number, number] },
      { t: 3000, pos: [0, 4, 11]  as [number, number, number] },
    ])
  })
  ...
  await page.evaluate((url: string) => {
    ...
    a.volume = 1.0
    a.play().catch(e => console.error('[v12] Audio play failed:', e))
  ...
  // ── Wait for approach animation to complete (3.5s from start = 3s anim + 0.5s buffer) ──
  // We've already waited 2s (0.5s settle + 1.5s for audio), so wait 1.5s more
  await sleep(1500)
  log('Approach animation complete — transitioning to auto-rotate')

  // ── Set camera to exact final position and re-enable auto-rotate ──
  await page.evaluate(() => {
    const cam = (window as any).__haloCamera
    const oc = (window as any).__haloOrbitControls
    if (!cam || !oc) {
      console.error('[v12] Camera or OrbitControls not available!')
      return
    }
    // Set to exact position the animation ended at
  ...
  log('Auto-rotate enabled at [0, 4, 11], speed=0.12')
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    ffmpegProc.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        console.error('[v12] ffmpeg stderr (last 1000 chars):', ffmpegStderr.slice(-1000))
  ...
  if (existsSync(RAW_V12)) {
    const rawSize = (statSync(RAW_V12).size / 1024 / 1024).toFixed(1)
  ...
  const finalPath = FINAL_V12.replace(/\\/g, '/')
  ...
  if (existsSync(GREETING_WAV)) {
    ...
    try {
      ...
      console.error('[v12] Scale+audio failed:', err.stderr?.toString().slice(-500))
  ...
  // ── Check final file ──
  const outputPath = existsSync(FINAL_V12) ? FINAL_V12 : RAW_V12
  ...
  log('QA — extracting frames at 0s, 3s, 5s, 10s')
  const qaTimestamps = [0, 3, 5, 10]
  for (const ts of qaTimestamps) {
    const qaFrame = resolve(TEMP, `demo-v12-qa-${ts}s.jpg`).replace(/\\/g, '/')
    try {
      execSync(
  ...
  writeFileSync(TIMING_LOG, JSON.stringify({
    ...
    changes_from_v11: [
      'Camera approach: start far [0,10,20], glide to [0,4,11] over 3s',
      'Final position CLOSER: Y=4, Z=11 (was Y=5, Z=14)',
      'After approach, auto-rotate takes over seamlessly (no teleport)',
      '16s total: 3s approach + 13s auto-rotate',
    ],
    capture: {
    ...
    video: {
      raw_video: 'demo-v12-raw.mp4',
      final_video: 'demo-v12.mp4',
  ...
  // ── QA summary ──
  console.log('\n[v12] QA CHECKLIST:')
  console.log('  [ ] 0s: Far establishing shot (camera high and far)')
  console.log('  [ ] 3s: Arrived at closer position, cards readable')
  console.log('  [ ] 5s: Auto-rotate showing cards scrolling')
  console.log('  [ ] 10s: More cards scrolled past')
  console.log('  [ ] No teleport at the 3s transition')
  console.log('  [ ] Full screen, no taskbar')
  console.log('  [ ] Audio starts at ~2s')
